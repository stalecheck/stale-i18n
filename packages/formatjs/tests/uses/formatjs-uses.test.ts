...
function buildOptions(caseDir: string, options: Partial<FormatjsCheckOptions> = {}) {
  return {
    target: path.join(caseDir, "src"),
    catalogs: path.join(caseDir, "locales", "{locale}.json"),
...
describe("formatjs public API use cases", () => {
